...
Const
	ccExprPunctuation : Array[1..20] Of String = (
		'+', '-', '*', '/',
		'div', 'mod', '(', ')', 
		'not', 'and', 'or', 'xor',
		'=', '>', '<','<>',
...
Type
	EExprScannerError = Class(Exception);
...
Begin
	Raise EExprScannerError.Create(IntToStr(fRow) + ',' + IntToStr(fCol) + ',' + aMsg);
End;
...
Begin
	...
	Else
		AppendToOutput(lLiteral, tkExprIdentifier);
End;

Procedure TExprScanner.ScanString;
Var
	lLiteral : String;
Begin
	lLiteral := '';
...
Begin
	Raise EExprParserError.Create(IntToStr(fRow) + ',' + IntToStr(fCol) + ',' + aMsg);
End;
